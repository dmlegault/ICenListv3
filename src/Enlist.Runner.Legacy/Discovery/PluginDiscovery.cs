...
/// match anything on this side of the boundary. Identical logic to the modern Enlist.Runner's own
/// PluginDiscovery — CustomAttributeData-based reflection is available unchanged on net472.
/// </summary>
public static class PluginDiscovery
{
    ...
    private static MethodInfo? FindMethod(Type type, string attributeSimpleName, List<string> warnings)
    {
        ...
        foreach (var method in type.GetMethods(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            {
                warnings.Add($"{type.FullName}: multiple methods carry [{attributeSimpleName.Substring(0, attributeSimpleName.Length - "Attribute".Length)}] - using {found.Name}, ignoring {method.Name}.");
                continue;
            }
