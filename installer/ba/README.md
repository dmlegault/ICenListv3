...
| `Enlist.Installer.Detection.Tests` | `net10.0` | 155 cases over the above, several against this machine. |
...
**`ContainerEngineDetection`** probes `wslc` and `docker` with a 10 second timeout. Neither is required, and none is a perfectly good answer. **`DefaultEngine` proposes `wslc` when it answers, then Docker, then nothing.** Through a reboot, `tools\probe-container-engines.ps1 -ArmStartupProbe` had `wslc` answering SYSTEM - listing, loading and running the runner image - at every sample, while Docker Desktop, an application in a user's session, never started; an unattended agent needs the engine that is there after a 3 a.m. patch reboot. For a few hours before that the default was Docker and never `wslc`, because `wslc` keeps a separate image store for every account and an image the operator loads is invisible to a LocalSystem agent; `InstallPlan.AgentEngineWarning` put a red note on it. Both are gone: the agent now loads the image from its `Images` folder itself, as its own account (Installer-UI-Design section 12 items 5 and 11), which answers the per-account store for either engine.
...
**Choosing a container engine brings up the runner image note.** The installer does not carry the runner image — it is a separate download, `enlist-runner-<version>.tar` — and the agent never pulls one, so an agent installed with an engine and no image fails every container application. What the agent needs is the download in its `Images` folder - it loads it itself, as its own account - and **when the download is beside the setup the bootstrapper puts it there**: `InstallPlan.SetupFolder` is read from Burn's `WixBundleOriginalSourceFolder` (the folder setup was started from, not the package cache it runs out of), `RunnerImageDownloadBesideSetup` finds the file, and `PostInstall` adds a `StageRunnerImage` step - optional, after the agent package runs and before enrollment - that copies the `.tar` and its `.sha256` into the folder the agent package created (never one of its own - see "After the packages"). So the Agent page replaces its grey "leave the engine empty" hint with an amber note that says which it is - *found beside this setup, it will be copied to …\Images and the agent loads it*, or *put it beside this setup, or copy it to …\Images later* - and the Finish page repeats it in full after a successful install. Both come from `InstallPlan.RunnerImageNote` (brief and full), so the wording is tested and follows what was typed; an image that is not `enlist/runner:<tag>` gets told to load *that* image as the agent's account, since the `Images` folder only takes runner images. The window is 660 high rather than 620 for it: the note first ran past the bottom of the Agent page, which does not scroll.
...
| `StageRunnerImage` | *(in-process)* copies `enlist-runner-<version>.tar` and its `.sha256` from beside the setup into the agent's `Images` folder | the agent package **ran**, it has a container engine and an `enlist/runner` image, and the download is beside the setup |
...
**Only enrollment and staging the runner image are optional**, and that is about what a failure means rather than importance. A schema that will not apply means the control plane cannot work. Enrollment failing usually means the control plane is not running yet — which on a single-box install it certainly is not, because every service here is created stopped. And without the runner image every process application still runs; only containers wait for the file.

**`StageRunnerImage` never creates the `Images` folder.** `Enlist.Agent.msi` creates it, locked to SYSTEM and Administrators, because whoever can write there chooses the image the LocalSystem agent loads. A folder the bootstrapper made would inherit `%ProgramData%`'s *Users may create files* — which is exactly what the first live run through `wslc` found, when the package had skipped creating it on a reinstall (`installer\README.md`, the Images folder checks). So a missing folder fails the step with that reason, rather than being papered over.
