...
Still outstanding: `verify.ps1 -Live` proves upgrade and uninstall for the agent package only, not all three; nothing is code-signed (section 12 item 7, which needs a certificate purchase rather than a design); and `live-e2e.ps1` has not been run under a named agent account, only LocalSystem. Through `wslc` it passes every check: the agent, as LocalSystem, loads the runner image from its `Images` folder and runs the container from it (section 12 item 11). Its first run there failed only on that folder's ACL, which reinstalls had been skipping - fixed, and now checked by `verify.ps1`. The wizard defaults to `wslc`, then Docker: through a reboot, `tools\probe-container-engines.ps1 -ArmStartupProbe` had `wslc` answering SYSTEM, loading and running the runner image at every sample, while Docker Desktop - an application in a user's session - never started. `wslc`'s per-account image store, which briefly made Docker the default, is answered by the agent loading the image from its `Images` folder as itself. `live-e2e.ps1` proves that path by default now; it has proved the Docker one.
