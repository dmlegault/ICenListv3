...
| Path | What it is |
|---|---|
| `src/` | The five components — `Enlist.Portal` (Blazor Server UI), `Enlist.ControlPlane` (REST + SignalR + database), `Enlist.Agent` (one per machine), `Enlist.Runner` (hosts one application; `.Legacy` is the net472 build), `Enlist.Deploy` (the publish CLI), plus `Enlist.ControlPlane.Contracts` |
...
| `tests/` | Four test projects plus `Enlist.TestSupport`, which spawns real control-plane, agent and runner processes rather than mocking them |
