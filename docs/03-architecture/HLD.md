...
| `Authentication/` | The bearer scheme (`EnlistBearerHandler`: three token kinds by hash, claims), the endpoint policy table and its fail-closed handler, the listener rules (`Off` only on loopback; no plain HTTP off loopback under `Required`), the management CLI verbs, the Operator-only key and join-token endpoints (`AccessEndpoints`, the same `CredentialIssuer` as the verbs) and the audit line on every administrative write (`AuditMiddleware`) — [Authentication-Design.md](Authentication-Design.md). |
...
| Module | Responsibility |
|---|---|
| `Services/ControlPlaneApiClient.cs` | The one HTTP client wrapper for every control-plane call the portal makes, plus its own local DTO mirror of the agent's status snapshot shape (deliberately decoupled from the control plane's opaque-blob storage — see [`SAD.md` §5](SAD.md#5-cross-cutting-design-decisions)). Presents the portal's key, names the person in `X-Enlist-Operator`, and refuses a Viewer's write before it leaves. |
| `Services/PortalAuthentication.cs`, `PortalRoles.cs`, `PortalCredential.cs` | Windows sign-in (Negotiate) and the two group-to-role policies — or, under `Off` on loopback, policies that admit everyone; a Windows identity turned into a detached principal with role claims once per request; the portal's own Operator key, read from configuration in the clear or as a `dpapi:` value (`Enlist.Portal.exe protect`). |
