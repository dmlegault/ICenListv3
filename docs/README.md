...
There are 27 documents here. **You do not need most of them to be productive.** This page is the
...
| [`0S-PreSales/`](0S-PreSales/) | Go-to-market material; each is a standalone page to open in a browser. [enList-Sales-Readiness.html](0S-PreSales/enList-Sales-Readiness.html) is the 15 September 2026 verdict on where enList stands for selling licenses — where it wins, where a buyer's evaluation stalls, and why program workloads ([Program-Workloads-Design.md](03-architecture/Program-Workloads-Design.md)) are the next move. [enList-Run-What-You-Have.html](0S-PreSales/enList-Run-What-You-Have.html) is the customer-facing story for that move: an annotated mock-up of the portal running VB6, VBScript, PowerShell and batch programs unchanged, the whole integration for one of them, and the case against Task Scheduler, NSSM and remote desktop. A concept — the feature is designed, not built — with a fictional customer. [enList-Ten-Sprints-to-Sellable.html](0S-PreSales/enList-Ten-Sprints-to-Sellable.html) is the roadmap between the two: ten ordered two-week sprints, each with its scope, the issues it solves, its value, the tests that prove it done, and what it unlocks next. Sprints 2–5 follow the program workloads design's phases. |
