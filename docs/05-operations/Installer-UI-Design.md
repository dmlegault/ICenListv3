...
11. **Which container engine an installed agent should use: `wslc`, with the agent loading the image as itself.** *(Found by installing for real, 2026-09-14; corrected, then decided, the same day.)*
    ...
    **Still open:**
    - **A full agent deploy through `wslc` as LocalSystem, from the Images folder.** `live-e2e.ps1` now does exactly this by default (`-ContainerEngine wslc`; `-ContainerEngine docker` for the Docker path): it removes the image from SYSTEM's store, installs from `out\` with the download beside the setup, and checks the installer copied it in, the folder's ACL, that the agent loaded it (`loaded.json`, its log, SYSTEM's `wslc image inspect`), and that the container runs from it and stops again. Until that run passes, the agent publishing the control port and mounting its package through `wslc` as LocalSystem is proved only by desk tests.
