...
| Containers | the detected engine, else *None* | Radios enabled per §4 detection. Sets `--container-engine` and `--container-image`. Note under the group: *net472 applications cannot run in a container* (Deployment-IaC). The image default is **`enlist/runner:<product version>`**, which `installer\build.ps1` builds and saves beside the installer as the separate `enlist-runner-<version>.tar` download (§12 items 5 and 8). The installer does not carry it and the agent never pulls it, so **once an engine is chosen the page shows an amber note with the load command** (`docker load -i enlist-runner-3.0.0.tar` / `wslc load -i …`) in place of the process-isolation hint (built 2026-09-14) | `AGENT_ENGINE` = `none | wslc | docker`, `AGENT_IMAGE` |
