...
| Data | `%ProgramData%\enList\Agent` | Becomes `--data`: `Packages\`, `Runners\`, `Logs\` (the layout `.demo/README.md` documents), and `Images\`, the runner image drop folder (§12 item 5), which `Enlist.Agent.msi` creates with a protected ACL of SYSTEM and Administrators - plus the agent's own account when that is not LocalSystem | `AGENT_DATADIR` |
...
| Containers | `wslc` when it answers, else Docker, else *None* (§12 item 11) | Radios enabled per §4 detection. Sets `--container-engine` and `--container-image`. Note under the group: *net472 applications cannot run in a container* (Deployment-IaC). The image default is **`enlist/runner:<product version>`**, which `installer\build.ps1` builds and saves beside the installer as the separate `enlist-runner-<version>.tar` download (§12 items 5 and 8). The installer does not carry it and the agent never pulls it, so **once an engine is chosen the page shows an amber note** in place of the process-isolation hint: when the download is beside the setup, that it will be copied into the agent's `Images` folder and the agent loads it; when it is not, where to put it (`InstallPlan.RunnerImageNote`, built 2026-09-14) | `AGENT_ENGINE` = `none | wslc | docker`, `AGENT_IMAGE` |
...
Progress per package. Finish offers *Start services now* (checked) and *Open the portal* (`http://localhost:5231`, only when the portal was installed). If the schema was skipped (§6.5) or verification was overridden (§6.7), Finish repeats that in red — those are the two things that will make the fleet look broken an hour later. An agent installed with a container engine gets the runner image note again, in full, after a successful install: either that the download was found beside the setup and copied into the agent's `Images` folder, or that it has to be copied there before any container application can start (§6.7, §12 item 5).
...
5. **Where do runner images come from for an agent install?** ~~Bundled tarball, loaded at install.~~ **Decided 2026-09-14: a separate download that operators side-load themselves** (`docker load -i` / `wslc load -i`) into whichever engine, on whichever machines, they run container-mode agents. The installer does not carry the image and does not load it, so it is ~200 MB smaller than a bundled image would have made it, and an agent that runs applications only as processes never handles the image at all.
   What follows from it: the agent runs every container with `--pull never` (since 2026-09-14; item 8 says why), so an agent whose machine does not have the image reports it missing by name rather than fetching anything. The download is the image `installer\build.ps1` builds, `enlist/runner:<product version>`, saved into `out\` beside the installers as `enlist-runner-<version>.tar` (about 80 MB) with a `.sha256` in the format `sha256sum -c` reads; `verify.ps1` checks the file loads under the tag the agent package names and was built from the current runner source. An `enlist-runner-<version>-README.md` beside it says where the file goes, and the load commands for loading it by hand.

   **Revised the same day: operators hand the file over, and the agent loads it.** Item 11 found that `wslc` keeps an image store per account, so `wslc load -i` in the operator's shell loads the image where a LocalSystem agent never looks. So the load moved into the agent (`RunnerImageDropFolder`):
   - **`<data>\Images`** (`C:\ProgramData\enList\Agent\Images` by default) is the drop folder. Before every container start the agent loads each `*.tar` there it has not loaded yet into its engine, *as its own account* - the one store that matters, for either engine and any service account, without anyone needing to know which account that is. What was loaded is recorded in `loaded.json` by name, size, time and engine; a changed file loads again, and a container that fails for want of its image makes the agent forget the record, so the retry reloads.
   - **It loads only `enlist/runner:*` archives**, refusing any other tag, and an archive with a `.sha256` beside it has to match. The folder's ACL is the real control - `Enlist.Agent.msi` creates it protected, SYSTEM and Administrators only (and the agent's account when that is not LocalSystem), because whoever can write to it chooses what LocalSystem loads, and `%ProgramData%` would otherwise let any user create files there. The tag check narrows what even an administrator's mistake can do.
   - **The installer copies the download in when it is beside the setup** (`WixBundleOriginalSourceFolder`), after the agent package runs and before enrollment - an optional post-install step, since every process application runs without it. So handing out `out\` as it is means no manual step at all.
   - The wizard's amber notes say which of the two it is (item 5's words are `InstallPlan.RunnerImageNote`, tested, following the engine and image as typed): *found beside this setup, will be copied to …\Images, and the agent loads it itself*, or *put the file beside this setup, or copy it to …\Images later*. A custom image (not `enlist/runner`) keeps the load-it-yourself note, naming the account.
...
8. **What tag does a released runner image carry?** **`enlist/runner:<product version>`, i.e. `enlist/runner:3.0.0`** — which is what §6.7 and the silent example assume. **Built since 2026-09-14:** `installer\build.ps1` builds the image under that tag, labelled with the version and a hash of the source it came from, and `verify.ps1` checks the tag the agent package names exists, carries that version, and is current. Until then nothing produced it, so every container-mode agent pointed at an image that existed nowhere.
   ...
   Getting the image onto an agent's machine is item 5: a download the installer copies into the agent's `Images` folder, and the agent loads.
...
11. **Which container engine an installed agent should use: `wslc`, with the agent loading the image as itself.** *(Found by installing for real, 2026-09-14; corrected, then decided, the same day.)*

    `installer\live-e2e.ps1` deploys the sample application to the installed agent, as a process and in a container, and both run. The container half first ran on **Docker**: the agent service, as LocalSystem, reaches Docker Desktop's engine (its pipe grants SYSTEM full control) and ran the image the script had side-loaded *as the operator* - because Docker is one engine with one image store, shared by every account.
    ...
    **What is true is that `wslc` keeps a separate image store for every account.** Its layers live under the account's `%LOCALAPPDATA%` - for LocalSystem, `C:\Windows\System32\config\systemprofile\AppData\Local` - and LocalSystem's `image list` came back empty while the operator's normal session held the demo's images. (The operator's own *elevated* shell also saw none of the containers running in their normal session. Only containers were listed there, not images, and an elevated token shares the account's `%LOCALAPPDATA%`, so that says the *sessions* are separate, not the image stores.) So the obvious step, `wslc load -i` in the operator's shell, puts the image where a LocalSystem agent never looks, and every container application fails with the image missing.
    ...
    A way round it that was considered and not taken: `.wslconfig` can point the container VM's storage at a host directory both the operator and the service can reach, so one `wslc load` serves both. Whether a store shared between a user and LocalSystem is safe to use concurrently, and what it needs in permissions, is untested - and it would be one more machine-wide setting for an operator to get right.

    For a few hours the wizard therefore defaulted to Docker and put a red note on `wslc` for a service account. **Then the startup probe settled which engine an unattended agent can actually rely on.** `probe-container-engines.ps1 -ArmStartupProbe`, across one reboot, asked both engines as SYSTEM every two minutes:
    - **`wslc` answered at every sample from two minutes after boot** - `list`, `load` of the runner image download into SYSTEM's own store in about 2 s, and `run` of it in about 1.5 s.
    - **Docker failed at every sample.** Docker Desktop is an application in a user's session, and it did not start: its *start when you sign in* setting was off. Even with it on, it starts when somebody signs in - and a server that reboots for patching at 3 a.m. has nobody signing in.
    - One caveat: a user signed in 15 seconds after boot, so "nobody has ever signed in" itself was not observed. `wslc` is a system service and did not need that session; Docker Desktop could not have run without one.

    So **the default is `wslc`**, and the per-account store is answered by moving the load into the agent (item 5): the agent loads the download from its `Images` folder into whichever store it runs as.
    - **The default engine is `wslc` when it answers, then Docker, then none** (`ContainerEngineDetection.DefaultEngine`).
    - **The red note is retired** (`InstallPlan.AgentEngineWarning` is gone): with the agent doing the load, choosing `wslc` for LocalSystem is no longer a trap. The Agent and Finish pages show item 5's amber image note for either engine.
    - The Prerequisites page lists both engines as *container isolation (optional)*.
    - Docker stays fully supported, and is the right choice where Docker Desktop is already how a machine runs containers and someone is signed in.

    **Proven on 2026-09-14 (18:21):** `live-e2e.ps1` now deploys through `wslc` by default (`-ContainerEngine docker` for the Docker path). It removed the image from SYSTEM's store, installed from `out\` with the download beside the setup, and the installer copied it into `Images`; the agent - LocalSystem, started by the SCM - loaded it into SYSTEM's `wslc` store itself (`loaded.json`, its log, `wslc image inspect` as SYSTEM), ran the sample in a container from it with its service Running, and stopped it when the rule was removed.

    **That run also found the folder's ACL missing**, and it was an installer defect rather than a check that was wrong. The `Images` lock first lived in the data folder's component, which is `Permanent` and `NeverOverwrite` with a registry key path. That key outlives an uninstall, so every later install - reinstall *or upgrade* - logged *Disallowing installation of component: AgentDataFolder since the registry keypath exists* and skipped it, CreateFolder rows and all; the folder was then created by the bootstrapper's copy, inheriting `%ProgramData%`'s *Users may create files*. The lock is now a component of its own that installs every time (so does the named-account grant, which the lock would otherwise wipe), the bootstrapper refuses to create the folder itself, and `verify.ps1` checks that no component creating it is Permanent or NeverOverwrite. **The rerun at 18:28 passed every check** - on that same machine, so on exactly the reinstall path that had skipped the lock.

    **Still open:**
    - **Whether a named user account's service logon sees that user's `wslc` images.** With the agent loading the image itself this matters less - it loads into whatever store its account has - but it is not proved.
