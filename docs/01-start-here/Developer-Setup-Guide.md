...
- `--join-token <token>` is needed only on the first start against a control plane whose authentication is `Required` — the default outside `Development` (`appsettings.Development.json` sets `Off`, so a local control plane on loopback needs none). The agent exchanges it for its own credential, stored DPAPI-protected at `<data>/credential`, and never needs the option again; left on a later start it is ignored with a warning. Mint one with `dotnet src/Enlist.ControlPlane/bin/Debug/net10.0/Enlist.ControlPlane.dll create-join-token` — see [`Authentication-Design.md` §4](../03-architecture/Authentication-Design.md).
