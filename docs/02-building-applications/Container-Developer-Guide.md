...
**`wslc` keeps a separate image store for every account** — the layers live under that account's `%LOCALAPPDATA%`, which for LocalSystem is `C:\Windows\System32\config\systemprofile\AppData\Local`. An installed agent runs as a Windows service under LocalSystem, whose store starts empty, so an image you `wslc load` in your own shell is invisible to it and every container application fails with the image missing. `wslc` itself works fine as LocalSystem: loading the runner image and running it took five seconds (`installer\tools\probe-container-engines.ps1 -Wslc`, 2026-09-14) — the image just has to be loaded *as* LocalSystem. An agent started from a terminal, as the demo does, is unaffected because it runs as you. **An installed agent loads the image itself**: put `enlist-runner-<version>.tar` in its `Images` folder (`C:\ProgramData\enList\Agent\Images` - the installer does it when the file is beside the setup) and the agent loads it into its engine, as its own account, before its next container start. That is why the installer now defaults to `wslc`: it answers a LocalSystem service after a reboot with nobody signed in, where Docker Desktop, an application in a user's session, does not. See Installer-UI-Design §12 items 5 and 11.
...
**The agent never pulls an image.** Every container it starts is run with `--pull never`, so the image has to be on the machine already — built as above, or loaded (`docker load` / `wslc load`). A missing one fails the start with a message saying so, rather than the engine fetching `enlist/runner:<tag>` from Docker Hub, a namespace enList does not own. `installer\build.ps1` builds the release tag, `enlist/runner:<product version>`, which is what an installed agent is configured with. The installer does not carry that image: `build.ps1` saves it into `installer\out\` as `enlist-runner-<version>.tar`, which is handed out beside the setup. The installer copies it into the agent's `Images` folder and the agent loads it, as its own account; only `enlist/runner:*` archives are loaded from there. A custom image is still yours to load, as the account the agent runs as (`docker load -i …` / `wslc load -i …`).
