...
**It is not yet safe to run outside a trusted network, and it still has defects that will bite in production.** The most severe remaining is structural: the absence of any authentication, which is a documented decision but means every agent will execute whatever any network peer uploads.
...
> **This is a design decision to make, not a patch to apply.** The natural shape given the architecture: a per-agent bearer credential issued at registration and required on every agent endpoint and on hub connect (binding `JoinAgentGroup` to the authenticated identity); a separate operator credential for the portal and the mutation endpoints; and TLS on the control plane. Until then, "trusted network boundary" should be the first line of the Deployment guide, not a footnote in the BRD.
...
- **Where the security boundary is** — see C2.
...
195 tests across five projects, all passing (148 at the time of the review). The posture is the project's strongest engineering asset: nothing at a process, database or engine boundary is faked. Coverage is strongest where the design is most novel — transports, the backend seam, containers, package distribution, conflict resolution, and now concurrent dispatch and the host's colliding callers — and thinnest where the remaining findings sit.
...
| Control plane API | Packages, registry, self-tag, agreement/conflict, exclusion, package and report retention, flavor/isolation rules, ports, endpoint feed, digest and name validation, unknown-digest refusal, uploads over the stock body limit | — |
...
- **Deployment-IaC** should lead with the trusted-network assumption and the consequence in C2.
- **BRD §5.2** "Out of scope" is accurate; C2 argues it should move to "Required before production" with a one-line design.
...
1. **C2** — decide the authentication model. It is the gate on running this anywhere but a lab, and the architecture makes the per-agent-credential shape straightforward. Everything above it can ship without it; nothing should ship past a trusted network without it.

Nothing in this list changes the architecture. The system's shape is right; the remaining work is deciding where the trust boundary sits.
