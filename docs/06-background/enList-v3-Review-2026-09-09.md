...
| C2 | Authentication, in three steps on 2026-09-11 ([Authentication-Design.md](../03-architecture/Authentication-Design.md)). Control plane: a bearer scheme for three token kinds, a fail-closed policy table, two startup rules (`Off` only on loopback; no plain HTTP off loopback), key and join-token management, an audit line on every administrative write. Agent: join-token enrollment, a DPAPI-protected credential under a closed ACL, the credential on every call and on the hub, a rejection said once and nothing stopped. Portal and tools: Windows sign-in, two groups to two roles, write controls hidden and refused for Viewers, the portal's own key stored protected, `X-Enlist-Operator` naming the person, `--api-key` on `enlist-deploy`. Pinned by `AuthenticationTests`, `AccessEndpointsTests`, `AgentCredentialTests`, `PortalAuthenticationTests`, `PortalRolesTests`, `ControlPlaneApiClientTests` and `DeployCliTests`. |
...
**Nothing in this review remains open.** The one structural finding — no authentication — was closed on 2026-09-11 in three steps ([Authentication-Design.md](../03-architecture/Authentication-Design.md)): every request to the control plane is authenticated by default, and authentication can be off only on loopback; agents hold their own credentials; the portal signs people in with Windows and refuses a Viewer's writes; tools present keys; every administrative write is audited with the key and the person. The demo runs with authentication off because it lives on loopback, which is the rule, not an exception to it.
...
Nothing is open. Every finding is in the *Resolved* table at the top; the sections below keep the analysis that led to each.
...
### 3.2 Security boundary (C2 — resolved)

> **Design decided 2026-09-11:** [Authentication-Design.md](../03-architecture/Authentication-Design.md) - Windows authentication for operators, join-token enrollment for agents, TLS at Kestrel, and authentication that can only be off on loopback. **Implemented in full on 2026-09-11** — the control plane (`AuthenticationTests`, `AccessEndpointsTests`), the agent (`AgentCredentialTests`) and the portal and tools (`PortalAuthenticationTests`, `PortalRolesTests`, `ControlPlaneApiClientTests`, `DeployCliTests`). **C2 is resolved.**
...
- **Where the security boundary is** — decided (C2): at the control plane's listener, with a credential per agent and a Windows identity per operator. Built on all three.
...
*Good:* the dialogs are honest about what they know and when ("read once on open and does not refresh — close and reopen"); the toggler names the blast radius and says "and future"; the wizard previews conflicts and port collisions before saving and still lets the operator proceed; the upload dialog scans the uploaded package and removes an empty one immediately. The portal has `UseHttpsRedirection`, HSTS and antiforgery; the control plane, since C2 step 1, refuses plain HTTP off loopback outright rather than redirecting it, which is the stronger of the two. Portal tests covered the resolution logic only at the time (§5).

*Since the review (C2 step 3).* A Windows identity becomes a detached principal with role claims once per request (`PortalRoles`), so a circuit never holds the access token a `WindowsIdentity` dies with; every `AuthorizeView` and `[Authorize]` is then a plain role check, the same under `Off`, where both policies admit everyone. `ControlPlaneApiClient` refuses a Viewer's write before sending it, because the portal's key is Operator-tier whoever is looking. One thing to watch: `UseStatusCodePagesWithReExecute` re-executes a 401 through a page that is itself protected, which adds a second `WWW-Authenticate: Negotiate` header — harmless to a browser, visible in a trace.
...
232 tests across five projects, all passing (15 skip themselves where the machine cannot run them: Docker, and Windows sign-in to the local machine) (148 at the time of the review). The posture is the project's strongest engineering asset: nothing at a process, database or engine boundary is faked. Coverage is strongest where the design is most novel — transports, the backend seam, containers, package distribution, conflict resolution, and now concurrent dispatch and the host's colliding callers — and thinnest where the remaining findings sit.
...
| Control plane API | Packages, registry, self-tag, agreement/conflict, exclusion, package and report retention, flavor/isolation rules, ports, endpoint feed, digest and name validation, unknown-digest refusal, uploads over the stock body limit; since C2 step 1: anonymous reaches `/health` only, join-token enrollment and its name binding, roles on keys, hub group binding, both startup rules; key and join-token management, credential revocation, the audit line | — |
| Portal | `PolicyConflictDetector`, `TagSelectorMatcher` and `IsolationSpec.WithMode` as plain xUnit; the real portal as a process: the Negotiate challenge, `Off` on loopback and refused off it, a Viewer's write refused in the client, group-to-role mapping, both policy sets, the `dpapi:` setting | No component tests; dialogs, tables and the wizard are verified by hand. Windows sign-in end to end runs only where the machine admits its own NTLM (three tests skip here). |
...
1. ~~**C2** — decide the authentication model~~ Decided and built on 2026-09-11 ([Authentication-Design.md](../03-architecture/Authentication-Design.md)). What remains is operational rather than a finding: certificates on real hosts, and the installer carrying the pages §14 of the design describes.

Nothing in this list changes the architecture. The system's shape is right, and the trust boundary is built on all three of its sides. What this project needs next is not a fix but an installer.
